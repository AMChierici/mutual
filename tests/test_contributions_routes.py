...
async def test_post_contribution_admin_records_and_writes_ledger(
    ...
    r = await admin_client.post(
        f"/pools/{pool.slug}/contributions",
        json={
...
async def test_post_contribution_unauthenticated_is_401(client, pool, members):
    r = await client.post(
        f"/pools/{pool.slug}/contributions",
...
async def test_post_contribution_non_admin_is_403(client, session, pool, members):
...
async def test_post_contribution_rejects_zero_amount(admin_client, pool, members):
    r = await admin_client.post(
        f"/pools/{pool.slug}/contributions",
...
async def test_post_contribution_rejects_bad_period(admin_client, pool, members):
    r = await admin_client.post(
        f"/pools/{pool.slug}/contributions",
...
async def test_post_contribution_rejects_unknown_member(admin_client, pool):
    r = await admin_client.post(
        f"/pools/{pool.slug}/contributions",
...
async def test_get_bulk_form_admin_lists_all_active_members(
    ...
    r = await admin_client.get(
        f"/pools/{pool.slug}/contributions/bulk", params={"period": "2026-W01"}
    )
...
async def test_get_bulk_form_unauthenticated_is_401(client, pool):
    r = await client.get(f"/pools/{pool.slug}/contributions/bulk")
    assert r.status_code == 401


async def test_get_bulk_form_default_period_is_current_iso_week(admin_client, pool):
    r = await admin_client.get(f"/pools/{pool.slug}/contributions/bulk")
    assert r.status_code == 200
...
async def test_post_bulk_records_multiple(admin_client, session, pool, members):
    ...
    r = await admin_client.post(f"/pools/{pool.slug}/contributions/bulk", data=data)
    assert r.status_code == 200
...
async def test_post_bulk_skips_blank_and_zero_rows(admin_client, session, pool, members):
    data = {
        "period": "2026-W03",
        f"amount_{members[0].id}": "100.00",
...
async def test_post_bulk_rejects_invalid_period(admin_client, pool, members):
    data = {
        "period": "2026-13",
...
async def test_post_bulk_non_admin_is_403(client, session, pool, members):
    members[0].status = type(members[0].status).active
    session.commit()
    tok = create_login_token(session, members[0].user_id)
    auth_session = consume_login_token(session, tok.token)
    ...
    r = await client.post(f"/pools/{pool.slug}/contributions/bulk", data=data)
    assert r.status_code == 403
