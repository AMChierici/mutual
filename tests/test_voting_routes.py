...
# ---------------------------------------------------------------------------
# GET /pools/{slug}/claims/pending
# ---------------------------------------------------------------------------
async def test_get_pending_unauthenticated_is_401(client, pool):
    r = await client.get(f"/pools/{pool.slug}/claims/pending")
    assert r.status_code == 401


async def test_get_pending_lists_voting_claims(member_client, pool, voting_claim):
    r = await member_client.get(f"/pools/{pool.slug}/claims/pending")
...
async def test_get_pending_excludes_auto_approved(
    member_client, pool, auto_approved_claim
):
    r = await member_client.get(f"/pools/{pool.slug}/claims/pending")
    assert r.status_code == 200
...
async def test_get_pending_excludes_claims_member_already_voted(
    member_client, session, pool, voting_claim, members
):
...
async def test_get_pending_empty_renders_no_claims_message(member_client, pool):
    r = await member_client.get(f"/pools/{pool.slug}/claims/pending")
    assert r.status_code == 200
...
# ---------------------------------------------------------------------------
# POST /pools/{slug}/claims/{id}/vote
# ---------------------------------------------------------------------------
async def test_post_vote_approve_records_and_redirects(
    member_client, session, pool, voting_claim, members
):
    r = await member_client.post(
        f"/pools/{pool.slug}/claims/{voting_claim.id}/vote",
        data={"decision": "approve", "reason": "looks reasonable"},
        follow_redirects=False,
    )
    assert r.status_code == 303
    assert r.headers["location"] == f"/pools/{pool.slug}/claims/pending"
...
async def test_post_vote_reject_records_decision(
    member_client, session, pool, voting_claim, members
):
    r = await member_client.post(
        f"/pools/{pool.slug}/claims/{voting_claim.id}/vote",
...
async def test_post_vote_blank_reason_stored_as_null(
    member_client, session, pool, voting_claim
):
...
async def test_post_vote_unauthenticated_is_401(client, pool, voting_claim):
    r = await client.post(
        f"/pools/{pool.slug}/claims/{voting_claim.id}/vote",
...
async def test_post_vote_on_unknown_claim_is_404(member_client, pool):
    r = await member_client.post(
        f"/pools/{pool.slug}/claims/99999/vote",
        data={"decision": "approve"},
    )
...
async def test_post_vote_on_auto_approved_is_400(
    ...
    r = await member_client.post(
        f"/pools/{pool.slug}/claims/{auto_approved_claim.id}/vote",
        data={"decision": "approve"},
    )
...
async def test_post_vote_double_vote_from_same_member_is_400(
    member_client, pool, voting_claim
):
    await member_client.post(
        f"/pools/{pool.slug}/claims/{voting_claim.id}/vote",
        data={"decision": "approve"},
...
async def test_post_vote_invalid_decision_is_400(member_client, pool, voting_claim):
    r = await member_client.post(
        f"/pools/{pool.slug}/claims/{voting_claim.id}/vote",
        data={"decision": "shrug"},
    )
...
async def test_post_vote_observer_is_403(client, session, pool, members, voting_claim):
...
async def test_majority_threshold_flips_claim_status_via_http(
    ...
    for user_id in (admin.user_id, members[0].user_id, members[1].user_id):
        client.cookies.delete(SESSION_COOKIE)
        tok = create_login_token(session, user_id)
        auth_session = consume_login_token(session, tok.token)
        client.cookies.set(SESSION_COOKIE, auth_session.token)
        r = await client.post(
            f"/pools/{pool.slug}/claims/{voting_claim.id}/vote",
...
# ---------------------------------------------------------------------------
async def test_detail_shows_vote_form_when_pending(member_client, pool, voting_claim):
    r = await member_client.get(f"/pools/{pool.slug}/claims/{voting_claim.id}")
    assert r.status_code == 200
    assert "Approve" in r.text
    assert "Reject" in r.text
...
async def test_detail_hides_vote_form_for_auto_approved(
    ...
    r = await member_client.get(f"/pools/{pool.slug}/claims/{auto_approved_claim.id}")
    assert r.status_code == 200
    assert 'name="decision" value="approve"' not in r.text
