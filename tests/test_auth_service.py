...
def _pool(session, name="P", currency="USD"):
    p = Pool(name=name, currency=currency, governance_config={})
    session.add(p)
    session.commit()
    return p
...
def _member(session, pool, name="Ada", role=MemberRole.member, status=MemberStatus.invited):
    m = Member(pool_id=pool.id, display_name=name, role=role, status=status)
    session.add(m)
    session.commit()
    return m
...
def test_create_login_token_returns_unused_unexpired(session):
    p = _pool(session)
    m = _member(session, p)
    tok = create_login_token(session, m.id)
    assert isinstance(tok, LoginToken)
    assert tok.id is not None
    assert tok.member_id == m.id
...
def test_create_login_token_returns_unique_token_each_call(session):
    p = _pool(session)
    m = _member(session, p)
    a = create_login_token(session, m.id)
    b = create_login_token(session, m.id)
...
def test_consume_login_token_creates_session_and_activates_member(session):
    ...
    assert auth_session.member_id == m.id
